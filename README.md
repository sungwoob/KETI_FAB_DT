...
## 실행 방법
1. Node.js로 정적 서버 실행 (서버 스크립트와 `package.json`은 `public/` 하위에 있습니다.)
  ```bash
  cd public
  ...
  ```
   ...
   ```
2. 브라우저에서 `http://localhost:8000/`(Node 서버를 사용할 경우) 또는 `http://localhost:8000/index.html`(Python 서버) 을 열어 모델을 확인합니다.

### 특정 IP로 접근하고 싶다면?
- 서버 바인딩 IP를 바꾸고 싶다면 `HOST` 환경 변수를 지정해 실행하세요. 예시: `HOST=192.168.0.10 npm start`
- 같은 네트워크의 다른 장치에서 접속할 때는 브라우저 주소창에 `http://<서버 IP>:8000/`(또는 Python 서버는 `http://<서버 IP>:8000/index.html`)을 입력합니다.
