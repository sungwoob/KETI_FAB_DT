...
function resolvePath(urlPath) {
  ...
  if (relativePath.startsWith('node_modules/')) {
    const nodePath = path.join(rootDir, relativePath);
    return nodePath.startsWith(nodeModulesDir) ? nodePath : null;
  }

  const targetPath = path.join(rootDir, relativePath);
  return targetPath.startsWith(rootDir) ? targetPath : null;
}
